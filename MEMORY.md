...
## 2026-09-24 SUPERSEDED fixed-final-compactness interpretation

**Historical only. Do not use this entry for current claims. It is superseded by the branch-safe control below.**

The fine fixed-final-compactness refinement was initially treated as the terminal result of the
...
The historical result file is results/fixed_final_compactness_refinement_20260918.json,
generated by scripts/fixed_final_compactness_refinement.py in GitHub Actions
run 35387620727. It is superseded and retained only for provenance.
...
## 2026-09-24 authoritative branch-safe transition control

This entry overrides the earlier fixed-final interpretation and all cross-EOS
summaries from:
...
results/fixed_final_compactness_branchsafe_20260924.json, generated by
scripts/fixed_final_compactness_branchsafe.py.

All reference EOSs begin at common baseline compactness C=0.14 with positive
mass slope. At each transition location, the full sampled C(D) curve is used
to bracket roots; the first root continuously connected to D=0 is retained
only if the positive-mass-slope screen holds along the deformation path.

For C_final=0.10, the deepest retained/largest-response locations are
h_tr/h_c=(0.475,0.550,0.600) for Gamma=(1.70,1.85,2.00). For
C_final=0.12, they are (0.500,0.650,0.725). In all six cases the next
location fails the path screen, so these are boundary-adjacent rather than
interior universal maxima. Selected shell centers span r/R=0.368--0.476 and
m/M=0.235--0.675. Do not claim a universal vulnerable shell.

Scope correction: the broad latent-field calculations are cold, causal
function-space stress tests. They are not a nuclear-theory posterior and do
