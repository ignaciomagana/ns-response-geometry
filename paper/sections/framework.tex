...
For a two-dimensional observable plane such as \((\ln\Lamtwo,\ln\Ibar)\), the quotient by the one-dimensional sequence tangent leaves a single normal direction. Its response gives a local scalar measure of non-universality. In higher-dimensional observable spaces, the eigensystem of the projected operator defines a hierarchy of transverse directions.
...
\end{equation}
When the first line of Eq.~\eqref{eq:second-order} is suppressed, the bracketed term is the leading local source of finite scatter. Its contraction with the EOS covariance gives a prediction for how residual universality should degrade as the allowed EOS perturbations broaden.
